...
Alarm code format: ALM-0xNNXXXXXX
  NN     = subsystem byte (01=Memory, 02=Voltage, 04=Fan, 06=RAID Card,
                08=PCIe Card, 0C=Drive Bay, 0D=Memory Resize, 0F=Event Log,
                10=Mainboard, 12=Chassis, 1A=BMC, 1C=Battery, 1D=OS, 20=Boot,
                23=OS Boot, 28=Platform Alert, 29=Entity Presence, 2C=System,
                2D=Processor, 2E=Power Supply, 3E=PCIe Switch, 40=Disk,
                41=Disk Array, 66=Storage, 68=Health Event, 6C=GPU, 6D=NVMe)
  XXXXXX = alarm ID within that subsystem

Severity suffix embedded in alarm code mapping:
  0 = 紧急 (Critical)  → ERROR
  1 = 严重 (Major)     → ERROR
  2 = 轻微 (Minor)     → WARNING
  3 = 提示 (Info)      → INFO
...
This module provides:
  - AlarmCodeInfo: dataclass with decoded alarm metadata
  - ALARM_CODE_MAP:  dict keyed by "ALM-0xXXXXXXXX" → AlarmCodeInfo
  - decode_alm(code: str) → AlarmCodeInfo | None
  - parse_alm_entries(path) → (format_name, list[LogEntry], parse_errors)
  - is_alm_code(code: str) → bool
"""
...
FILE_PATTERNS: list[str] = []  # not file-based; called by app_debug/syslog parsers
...
# Alarm ID → (description, default_severity_override)
...
ALARM_DB: dict[str, tuple[str, str]] = {
    ...
    # ===== System (0x2C) - overall system events =====
    ...
    # ===== Atlas / NPU specific (from EDOC1100317321) =====
    ...
    # ===== Event / System Event (generic events) =====
...
ALM_RE = ...  # defined after ALARM_DB to allow forward ref
...
def decode_alm(code: str) -> Optional[AlarmCodeInfo]:
    ...
    # Look up with full 8-char form first
    ...
    subsystem_nn = padded[0:2]
    subsystem_name = SUBSYSTEM.get(subsystem_nn, f"Unknown(0x{subsystem_nn})")
    # Try to infer severity from last digit
